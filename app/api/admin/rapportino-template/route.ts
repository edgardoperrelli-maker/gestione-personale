...
export async function GET() {
  const { data, error } = await supabaseAdmin.from('rapportino_template')
    .select('id, nome, committente, campi, info_campi, titolo_campi, foto_id_priority, tipo, is_default, active, solo_manuale, created_at, updated_at')
...
export async function PATCH(req: Request) {
  ...
  for (const k of ['nome', 'committente', 'campi', 'info_campi', 'titolo_campi', 'foto_id_priority', 'tipo', 'active', 'solo_manuale'] as const) if (k in parsed.data) patch[k] = (parsed.data as any)[k];
