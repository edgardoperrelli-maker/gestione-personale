...
export async function GET() {
  const { data, error } = await supabaseAdmin.from('rapportino_template')
    .select('id, nome, committente, campi, info_campi, titolo_campi, is_default, active, created_at, updated_at')
...
export async function POST(req: Request) {
  ...
  const { data, error } = await supabaseAdmin.from('rapportino_template')
    .insert({ nome: parsed.data.nome, committente: parsed.data.committente ?? null, campi: parsed.data.campi, info_campi: parsed.data.info_campi, titolo_campi: parsed.data.titolo_campi, active: parsed.data.active }).select('id').single();
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
...
export async function PATCH(req: Request) {
  ...
  for (const k of ['nome', 'committente', 'campi', 'info_campi', 'titolo_campi', 'active'] as const) if (k in parsed.data) patch[k] = (parsed.data as any)[k];
