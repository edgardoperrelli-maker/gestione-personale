...
- **Reperibili per territorio della foglia (automatico per nome)**: la foglia È già un territorio, quindi `territoriDellArea` mappa per **nome** sui `territories` del cronoprogramma — Firenze→`FIRENZE`, Lazio Centro/Est→`LAZIO CENTRO`+`LAZIO EST` (accorpati), Perugia→`PERUGIA`. La tendina "Esecutore" mostra solo i reperibili di quei territori (`caricaReperibili(...,territoryIds)`). Nessuna selezione manuale (rimosso). La tabella `pi_aree_territori` non è più usata.
- **Badge "da approvare" in sidebar**: la voce "Pronto Intervento" mostra il conteggio richieste in attesa (come "Lista attesa"), via `useProntoInterventoCount` aggiunto al `RichiesteManualiProvider`. Le card del modulo mostrano il conteggio per foglia.
