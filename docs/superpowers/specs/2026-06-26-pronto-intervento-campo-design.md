...
- **Reperibili per territorio della foglia**: tabella di mappatura `pi_aree_territori` (foglia → territori del cronoprogramma). La tendina "Esecutore" del link mostra **solo i reperibili dei territori associati** alla foglia (`caricaReperibili(...,territoryIds)`); nessuna mappatura = tutti (retro-compatibile). Configurabile dal dettaglio foglia (bottone "Territori"). Migration `20260626000003_pi_aree_territori.sql`.
