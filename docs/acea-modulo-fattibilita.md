...
| 42 | Selezione a riga larga | Il click su **ODL, Attività, Impianto o Matricola** spunta la riga come il checkbox (shift-click per l'intervallo): il quadratino era un bersaglio da 16px. Le colonne dopo restano celle (cursore, copia, editor); su quelle d'identità il cursore resta raggiungibile con le frecce. La **copia dell'ODL nudo** — che questo click si era mangiato — torna con l'**icona al passaggio** sulla cella (click → appunti + toast, senza toccare la spunta) |
...
| 50 | Matricola del misuratore INSTALLATO (acqualatina) | Il flusso «ACQUALATINA SOSTITUZIONE MISURATORI» acquisisce un'azione **obbligatoria**: `matricola_nuova` — «MATRICOLA NUOVO MISURATORE», subito dopo LETTURA VECCHIO, che è l'ordine dei gesti sul posto. Nasce con essa il **tipo di campo `matricola`**: casella di testo con lo **scanner del codice a barre** accanto (`CampoInput` → `ScannerMisuratore`, lo stesso lettore di risanamento e limitazioni). Scansione e digitazione scrivono lo **stesso** valore nella stessa risposta: lo scanner è una scorciatoia, non un percorso a parte — fotocamera negata, buio o etichetta rovinata e l'operatore digita, senza restare fermo sul posto. Nessuna verifica a catalogo, al contrario del passo «cerca matricola» (dec. 46 e `CercaMatricolaAcqualatina`): quello controlla il misuratore **rimosso**, che nel master c'è; il **nuovo** non è in nessun elenco nostro. Obbligatoria **solo di fatto sulle voci positive** — `campiObbligatoriMancantiVoci` salta gli obbligatori con esito negativo, e un contatore non sostituito non ha una matricola nuova da leggere. Il valore va in MAIUSCOLO come gli altri campi liberi (`maiuscolaRisposteTesto`): «a12» e «A12» sono lo stesso misuratore. Migration `20260803160000` **chirurgica sul jsonb** `campi` (il flusso era già stato corretto a mano in prod — SARACINESCA da crocetta a select) e idempotente; i rapportini GIÀ generati non cambiano, perché `campi_snapshot` congela le azioni alla generazione: l'azione entra da quelli generati DOPO. **Nel registro** la risposta diventa la colonna «Matricola nuova», predefinita e subito accanto a «Matricola» (la domanda dell'ufficio è il passaggio dall'una all'altra): stessa meccanica di «Eseguito» nelle massive — valore letto da `rapportino_voci.risposte` sugli interventi della pagina, **senza imbuto né ordinamento** (non è una colonna del registro, filtrarla mentirebbe sul conteggio) e **decorativa** (se la lettura cade la tabella si carica lo stesso). Una sola lettura di `rapportino_voci` serve entrambe le colonne, ognuna solo sulla vista che la disegna. **Obbligatoria PER L'ESITAZIONE**, non solo per l'invio: una matricola obbligatoria vuota tiene la voce «da fare» anche con ESEGUITO = SI — gemello di `noteCompilate` sull'altro versante dell'esito (`matricoleObbligatorieCompilate` in voceColore.ts). Non è pignoleria sul modulo: una voce verde chiude l'intervento, chiude l'ordine nel registro e apre la riga a magazzino, e prima lo faceva con un buco al posto del pezzo montato — con l'obbligo che scattava solo all'INVIO, cioè a fine giornata, lontano dall'impianto. Motivo nuovo `matricola_mancante` (≠ `senza_esito`: l'esito c'è, e dirgli il contrario manda a cercare il campo sbagliato) in lista, banner e gate 409 del server
| 51 | Il negativo NON chiude (acqualatina) | Su questa commessa la chiusura la scrive il nostro motore, e la prima versione chiudeva su **qualunque** esito. Il 03/08 le **12 righe di via Tuccia** esitate negative sono finite in «Chiusi» con `aperto = false`, e riassegnarle a un altro operatore rispondeva «**ordine già chiuso su ACEA**» — un committente che con la commessa non c'entra, su lavoro vero ancora da fare. Ora vale l'invariante per intero: **chiude solo il POSITIVO** (il contatore è stato sostituito, e «ODL positivo = definitivamente chiuso» resta l'invariante dell'indice unique e di `pianoPianificazione`); il **negativo** — utente assente, contatore inaccessibile — è un **tentativo**, e lascia la riga APERTA con stato «**Aperta — non eseguita**», senza `data_completamento` (la riga non è chiusa; chi c'è già andato si legge in «Esecutore»/«Data pianificata»). Quella stringa è anche la **voce d'imbuto** con cui l'ufficio richiama la coda del ripasso in un clic — è il motivo per cui su acqualatina i due pezzi stanno nel DATO e non nella vista. Decisione pura in `lib/acqualatina/chiusuraRegistro.ts`; **nessuna migration**: le guardie della riconciliazione riaprono da sole le righe chiuse a torto (`esito_positivo is null or (false and aperto false)`) e non toccano MAI una chiusa positiva, restando idempotenti sul giro da 60s |
| 52 | Stato ED esito nella stessa cella | La colonna «Stato» diceva metà della storia: su ACEA «completato» sta sopra un lavoro riuscito e sopra un accesso negato (**3.303 righe su 5.293**), e per sapere quale delle due si doveva accendere «Esito ACEA», colonna non predefinita. Ora la cella porta l'**esito attaccato allo stato** — «completato — eseguito», «completato — non eseguito» — con **spunta o croce cerchiata** e verde/rosso: gli stessi segni della colonna «Eseguito» del modulo Interventi, che parla della stessa cosa dal lato nostro. Su **acqualatina** l'etichetta NON si ricompone: `stato_desc` la porta già intera (dec. 51), e ricomporla la direbbe due volte. Aperto = nero (non è un esito, è lavoro da fare), chiuso senza verdetto = smorzato. Colonna allargata (130 → 190/175): la seconda metà sarebbe stata la prima a sparire nel troncamento. Helper puro `statoOrdine` in `lib/acea/colonneTabella.ts`, quindi vale anche per l'**export della vista** e per la copia in appunti |
