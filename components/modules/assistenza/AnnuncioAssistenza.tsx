// components/modules/assistenza/AnnuncioAssistenza.tsx
'use client';
        ...
        <section>
          ...
          <ul className="space-y-1.5 text-[13px] text-[var(--brand-text-muted)]">
            <Voce t="Dall’operatore">
              {'sul suo rapportino tocca il salvagente '}
              <LifeBuoy size={13} className="inline align-[-2px] text-[var(--primary-text)]" aria-hidden />
              {' in basso a sinistra → “Chiedi assistenza”. La richiesta ti compare qui in alto, in “Richieste in arrivo”: premi Apri.'}
            </Voce>
...
function Voce({ t, children }: { t: React.ReactNode; children: React.ReactNode }) {
  return (
    <li className="flex gap-2">
      ...
      <span>
        <strong className="font-semibold text-[var(--brand-text-main)]">{t}</strong>: {children}
      </span>
    </li>
  );
}
