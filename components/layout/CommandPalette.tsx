  ...
  const entries = useMemo<Entry[]>(() => {
    ...
    for (const group of GROUP_ORDER) {
      for (const item of visibili.filter((i) => i.group === group)) {
        ...
        } else if (item.key === 'acqualatina') {
          // Come in Sidebar: due viste, due voci. Le etichette portano il committente perché in
          // palette si CERCA — «acqua» deve trovare entrambe, «misuratori» quella giusta.
          base.push(
            { href: '/hub/acqualatina/pianificazione', label: 'AcquaLatina — Pianificazione', group: groupLabels[group], icon: MODULE_ICONS.acqualatina },
            { href: '/hub/acqualatina/misuratori', label: 'AcquaLatina — Misuratori rimossi', group: groupLabels[group], icon: MISURATORI_ACQUALATINA_ICON },
          );
        } else {
          base.push({
            href: item.href,
