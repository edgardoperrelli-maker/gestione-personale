...
export default function EditAssignmentDialog({
  ...
  async function save() {
    ...
    const { data, error } = await sb
      .from('assignments')
      .update({
        day_id: assignment.day_id,               // richiesto dai trigger
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <label className="text-sm">
              ...
              <select className="w-full border rounded-lg px-3 py-2 bg-white" value={staffId} onChange={(e) => setStaffId(e.target.value)} autoFocus>
                <option value="">— Seleziona —</option>
                {staffSorted.map(s => (<option key={s.id} value={s.id}>{s.display_name}</option>))}
              </select>
            </label>

            <label className="text-sm">
              <span className="block text-gray-600 mb-1">Attività</span>
              <select className="w-full border rounded-lg px-3 py-2 bg-white" value={actId} onChange={(e) => setActId(e.target.value)}>
                <option value="">— Nessuna —</option>
                {actSorted.map(a => (<option key={a.id} value={a.id}>{a.name}</option>))}
              </select>
            </label>

            <label className="text-sm">
              <span className="block text-gray-600 mb-1">Territorio</span>
              <select className="w-full border rounded-lg px-3 py-2 bg-white" value={terrId} onChange={(e) => setTerrId(e.target.value)}>
                <option value="">— Nessuno —</option>
                {terrSorted.map(t => (<option key={t.id} value={t.id}>{t.name}</option>))}
              </select>
            </label>
          ...
          <label className="text-sm block">
            ...
            <input className="w-full border rounded-lg px-3 py-2 bg-white" value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Opzionale" />
          </label>

          {err && <div className="text-sm text-red-600">{err}</div>}

          <div className="px-0 pt-3 border-t flex items-center justify-end gap-2">
            <button type="button" onClick={onClose} className="px-3 py-1.5 rounded-lg border bg-white hover:bg-gray-50" disabled={saving || busy}>
              Annulla
            </button>
            <button type="button" onClick={handleDelete} disabled={busy} className="px-3 py-1.5 rounded-lg border bg-white hover:bg-gray-50">
              Elimina
            </button>
