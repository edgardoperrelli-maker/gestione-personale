...
export default function EditAssignmentDialog({
  ...
  onClose: () => void; onSaved: (row: Assignment) => void;
}) {
...
const availableStaff = useMemo(
  ...
  // Pulisci errore quando cambiano i campi
  useEffect(() => {
    setErr(undefined);
  ...
  async function save() {
    ...
    if (updErr) {
      setSaving(false);
      setErr(updErr.message || 'Errore di salvataggio');
      return;
    }

    // Rileggi con join per restituire lo shape atteso
    const res = await sb
      .from('assignments')
      .select(`
