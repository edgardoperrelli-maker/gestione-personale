...
describe('buildTemplateImport', () => {
  ...
  const MASTER: RigaMasterOdl[] = [
    { odl: '912000001', descrizione: 'Limitazione Massiva su Impianto', matricola: 'M1', impianto: '19633002', indirizzo: 'VIA ROMA 1', cap: '00030', comune: 'LABICO' },
    { odl: '912000002', descrizione: '', matricola: 'M2', impianto: '', indirizzo: '', cap: '', comune: 'RIANO' },
  ];
  ...
  it("con master: terzo foglio MasterODL protetto, righe nell'ordine dato", async () => {
    ...
    expect((wm.getRow(1).values as unknown[]).slice(1)).toEqual(['ODL', 'DESCRIZIONE ATTIVITÀ', 'MATRICOLA', 'INDIRIZZO', 'CAP', 'COMUNE', 'PDR / IMPIANTO']);
    expect((wm.getRow(2).values as unknown[]).slice(1)).toEqual(['912000001', 'Limitazione Massiva su Impianto', 'M1', 'VIA ROMA 1', '00030', 'LABICO', '19633002']);
  ...
  it('con master: MATRICOLA/Indirizzo/COMUNE/DESCRIZIONE si compilano dall\'ODL (VLOOKUP su MasterODL, self-locating, identico su ogni riga)', async () => {
    ...
    const attese: Array<[(typeof COLONNE_TEMPLATE)[number], number]> = [
      ['DESCRIZIONE ATTIVITÀ', 2], ['MATRICOLA', 3], ['Indirizzo', 4], ['CAP', 5], ['COMUNE', 6], ['PdR / Impianto', 7],
    ];
    for (const [nome, n] of attese) {
      const f2 = String((cella(ws, 2, nome).value as { formula?: string })?.formula ?? '');
      expect(f2).toContain(`VLOOKUP`);
      expect(f2).toContain(`${FOGLIO_MASTER_ODL}!$A$2:$G$3,${n},FALSE`);
